...
Create a small set of manual example chunks from the cleaned Boethius source.

These examples define what a good chunk should look like before creating an automated chunking script.

The goal is not to chunk the whole source manually. The goal is to create representative examples for later script validation.
...
## Chunk Quality Principles

A good chunk should:

- contain one coherent passage or idea
- preserve enough context to be understandable
- avoid cutting off mid-argument where possible
- be small enough to retrieve usefully
- include a stable chunk ID
...
Chunk ID: `BOETHIUS_CONSOLATION_001_CHUNK_001`

Source ID: `BOETHIUS_CONSOLATION_001`

Source file: `sources_public/processed/boethius_consolation_james_clean.md`
...
This passage introduces Philosophy as a dramatic and symbolic figure, contrasts her with the Muses of Poetry, and establishes the central therapeutic frame of the work: Boethius is spiritually sick and needs healing rather than emotional indulgence.
...
Chunk ID: `BOETHIUS_CONSOLATION_001_CHUNK_002`
...
This passage introduces Philosophy's critique of Fortune. It is a coherent argument about Fortune's mutability, the instability of external happiness, and why Boethius should not be surprised that Fortune abandons those who depend on her.
...
Strong candidate chunk. It contains one clear philosophical argument and would be useful for retrieval questions about Fortune, instability, false happiness, and the wheel of Fortune. It is somewhat long, but still coherent and not cut off mid-argument.

---

## Manual Chunk 003

Chunk ID: `BOETHIUS_CONSOLATION_001_CHUNK_003`
...
This passage defines happiness as the common goal of human striving and explains how human beings pursue it through wealth, rank, power, glory, pleasure, family, reputation, and bodily excellences.
...
Strong candidate chunk. It is conceptually dense but unified around one idea: all people seek happiness, though often through mistaken partial goods. It is useful for questions about true happiness, false goods, and Boethius's structure of desire.

---

# Initial Chunking Notes

## What looks easy to chunk

- The book has clear book/chapter/song divisions.
- Many prose sections contain coherent philosophical arguments.
- Some passages naturally fit into 300–800 word chunks.
- The source already has a stable source ID and cleaned Markdown format.

## What may be hard to chunk

- The work alternates between prose and verse, so the script may need to avoid mixing unrelated prose and song sections accidentally.
- Some arguments span multiple paragraphs and may be awkward if split purely by word count.
- Footnotes may need special handling so they do not pollute retrieval results.
- Some chunks may need section metadata later, such as Book I, Book II, etc.

## Rules the automated chunking script should follow

- preserve paragraph breaks
- avoid very tiny chunks
- avoid very large chunks
- avoid cutting in the middle of obvious sections when possible
- include `source_id` and `chunk_id` for every chunk
- include `word_count`
- include approximate start and end line numbers if practical
